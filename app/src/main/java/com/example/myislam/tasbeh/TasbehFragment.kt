package com.example.myislam.tasbeh
...
class TasbehFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        var mediaplayer = MediaPlayer.create(context, R.raw.subhan_allah)
        mediaplayer.start()
        viewBinding.counter.setOnClickListener{
            startTasbeh()
        }
        viewBinding.soundMute.setOnClickListener({
            mutesound()
        })
    }

    private fun mutesound() {
            val toast = Toast.makeText(context,"Sound Muted",Toast.LENGTH_LONG)
            toast.show()
    }
    ...
    private fun startTasbeh() {
        ...
        if (times_clicked == 33){
            ...
            mediaplayer.start()
            Toast.makeText(context,"التسبيحة المائة \n لا اله الا الله له الملك وله الحمد",Toast.LENGTH_LONG).show()
        }
    }
